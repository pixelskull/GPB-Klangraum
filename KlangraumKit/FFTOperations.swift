...
// shit
public func fft(setup: FFTSetup, x: [Double], fft_length: Int) -> (complex: SplitComplexVector<Double>, mag: [Double]) {
    var splitComplex = SplitComplexVector<Double>(count: count(x) / 2, repeatedValue: Complex<Double>(real: 0, imag: 0))
    var dspSplitComplex = DSPDoubleSplitComplex( realp: &splitComplex.real, imagp: &splitComplex.imag )
    ...
    // compare dspSplitComplex of both implementations
    // figure out why normalizedMagnitudes are still complex numbers instead of real values
