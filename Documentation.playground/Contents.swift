...
* **Sample:** Beschreibt einen Messpunkt innerhalb eines Signals. Im Programmatischen typischerweise ein Integer oder Float Wert, welcher den Amplitudenausschlag beschreibt.
...
Die Funktion *dematerialize() -> T* wird zum Auslesen des Wertes benötigt. Wie in der Implementierung zu sehen ist, handelt es sich bei dem Returnwert um einen *Optional*, was bedeutet, dass mittels der in Swift eingebauten *if let*-Schreibweise die Ergebnisse einfach evaluiert werden können (Success- und Failure-Handling). Da im Erfolgsfall der Wert des Identity-Monad zurückgegeben wird, macht die Einbindung des Failable-Enums in andere Projekte kaum Aufwand und verbessert, nach Meinung der Autoren, die Lesbarkeit und das Verhalten im Fehlerfall, vor allem bei einem funktionalen Programmierstil.
...
Wie die Magnitudes-Funktion gezeigt hat, werden einige weitere Funktionen benötigt, welche auf den Daten der FFT operieren. Da die meisten dieser Funktionen intern die Funktionen der vDSP API verwenden, um beispielsweise aus komplexen Zahlen (DSPSplitComplex) die Polarkorrdinaten zu extrahieren, wurden die Funktionen wiederum in Swift Funktionen gewrapped. Diese Funktionen in Swift haben meistens eine Signatur von *func<A>(SplitComplexVector<A>) -> Array<A>*, um erneut nicht von der vDSP API abhängig zu sein. Auch hier kann die Bibliothek für die digitale Signalverarbeitung ausgetauscht werden, ohne das Änderungen bei der aufrufenden Instanz durchgeführt werden müssen.
...
Die Mapping-Strategien werden benötigt, um die Informationen des Signals mit möglichst wenig Verlust auf einen kleineren Frequenzbereich abzubilden. Um dies zu realisieren, wurde die bestehende Aufnahme zuerst mittels linearer Interpolation auf einen höheren Samplingwert gesetzt. Bei diesem Verfahren werden neue Samples zwischen zwei bestehenden Samples mittels *linear Regression* errechnet und hinzugefügt. Nachdem die Aufnahme nun mehr Samples besitzt, in unserem Fall so viele, dass es einen größten gemeinsamen Teiler gibt, kann mittels Downsampling auf die gewünschte Samplerate bzw. in unserem Fall die *FFT Bins* heruntergerechnet werden. Durch das Upsampling und Downsampling soll der Verlust beim Umrechnen minimiert werden, da zu Beginn des Downsamplings mehr Informationen zur Verfügung stehen. Mehr zu diesem Ansatz im nächsten Kapitel.
...
Im Rahmen dieser Ausarbeitung wurde die Noise-Reduction sehr naive umgesetzt, weshalb diese in zukünftigen Projekten weiter ausgeführt werden sollte. Zum Demonstrieren der Funktionsweise soll dieser einfache Ansatz jedoch genügen.
...
Des Weiteren wurde eine Wrapperfunktion erstellt, mittels der auch das auf Stereo ausgesplittete Signal in Form eines zweidimensionalen Arrays gespeichert werden kann. Dies soll an dieser Stelle jedoch nicht weiter erläutert werden.
...
Zuletzt sei noch die Implementierung erwähnt, welche aus einer Audiodatei die Samples in Mono oder Stereo extrahiert und vice versa. Auch hierbei liegt eine C API zu Grunde, weshalb ein Swift-Wrapper, die Klasse AudioFile, implementiert wurde. Zudem findet sich in der iOS App ein Audioplayer, um die an die Klangwahrnehmung des Benutezrs angepassten Samples als Audio-Datei abzuspielen. All diese Schritte bilden den FFT-Stack, wie er in Kapitel *Beispiel anhand eines Anwendungsfalls* zu sehen ist. Das Design wurde so gewählt, dass die Verarbeitung möglichst performant ist, um die Audio-Datei unmittelbar nach dem Hörtest abzuspielen und keine zusätzlichen Wartezeiten zu dulden.
